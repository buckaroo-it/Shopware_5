...
class Shopware_Controllers_Backend_BuckarooGuaranteeRefund extends Shopware_Controllers_Api_Rest implements CSRFWhitelistAware
{
...
//                foreach ($refundedArticleIds as $itemID) {
//                    if (in_array($itemID, $capture->getItemsId())) {
//                        array_push($invoiceRefundItems[$x]['itemIDs'], $itemID);
//                    }
//                }
                    $x++;
                }
    ...
    public function executeRefund($request, $transaction, $refundValue, $class, $order, $invoice)
    {
        ...
        $request->setDescription($class->getRefundDescription($transaction->getQuoteNumber(), $order->getShop())); // description for on a bank statement
        ...
        if (is_null($invoice)) {
            $request->setServiceParameter('OriginalInvoiceNumber', $transaction->getQuoteNumber());
            $request->setAmountCredit($refundValue);
        } else {
            $request->setServiceParameter('OriginalInvoiceNumber', $invoice['invoiceNumber']);
        }
        $request->setServiceAction('CreditNote');
